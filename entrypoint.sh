...
echo ::set-env name=CI_COMMIT_REF_SLUG::$(echo ${GITHUB_HEAD_REF:-${_GITHUB_BRANCH:-$_GITHUB_TAG}} | sed 's/\(.*\)/\L\1/g' | sed 's/[^abcdefghijklmnopqrstuvwxyz0-9-]/-/g' | sed 's/\(^[[:digit:]]*[^[:alnum:]*\|^[^[:alnum:]]*\|[^[:alnum:]]*$$\)//g')
